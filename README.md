...
intents.message_content = True  # Mandatory for standard prefix commands
...
### Section
A **Section** arranges structural content horizontally, enabling regular texts on the left-hand partition and accessories (Thumbnail) fixed to the right-hand side.
```python
ui.Section(
    *text_components,   # TextDisplay items allocated to left frame
    accessory=...,      # A dedicated Thumbnail component locked to the right side
    id=None
)
```
...
section = ui.Section(
    ...
    accessory=ui.Thumbnail(
        media=discord.UnfurledMediaItem(url="https://example.com/logo.png"),
...
### Button
Clickable action nodes mapped inside active ActionRow elements.
```python
ui.Button(
...
└── Container (accent_color=None removes left border decoration)
    ...
    │   └── Thumbnail (Right partition accessory layout)
    ├── MediaGallery
    │   └── Image Collections
    ├── Separator
    └── ActionRow
...
| Section | TextDisplay components (Left) + Thumbnail component (Right Accessory) |
...
| **Section Confined Thumbnails** | Thumbnail structures must populate an accessory parameter slot within a Section. |
